...
class PX4Interface:
    ...
    def _euler_to_quaternion(roll, pitch, yaw):
        """Return the PX4/uORB quaternion order ``[w, x, y, z]``.

        ``VehicleAttitudeSetpoint.q_d`` is not ROS geometry_msgs order.  Keeping
        the order explicit is safety-critical: identity must be [1, 0, 0, 0],
        not [0, 0, 0, 1] (which PX4 interprets as a 180-degree rotation).
        """
