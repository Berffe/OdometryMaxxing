...
from bee_control.diagnostics.wind_layout import decode_wind_array
...
class BeeLandNode(Node):
	def __init__(self, config: BeeConfig | None = None):
		...
		self.create_subscription(
			Float32Array, cfg.topics.wind, self.on_wind, sensor_qos)
	...
	def on_wind(self, msg: Float32Array):
		try:
			wind = decode_wind_array(msg.data)
		except ValueError as exc:
			self.get_logger().error(f"Wind schema mismatch: {exc}")
			return

		receipt = self.time.receipt_stamp()

		self.diagnostics.write_wind(
			wind,
			receipt_wall_sec=receipt.wall_sec,
			receipt_monotonic_sec=receipt.monotonic_sec,
		)
	...
	def on_control_timer(self):
		...
		mc = self.mission.update(MissionInputs(
			...
			actuation=ActuationFeedback.from_control_law(
				self.control_law,
				self.state.setpoint.thrust,
				last_roll_cmd_rad=self.state.setpoint.roll,
				last_pitch_cmd_rad=self.state.setpoint.pitch,
			),
		))
