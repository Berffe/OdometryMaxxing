...
increases exponentially:
    ...
    radius(t) = radius_0 * exp(expected_lambda * t)

For a pure centered zoom field the physical expansion rate is therefore exactly
`expected_lambda` [1/s] -- that is lambda = -hdot/h, NOT the full 2-D divergence
2*lambda. Once the circle exceeds the image field of view the generator keeps
rendering the same surface closer and closer, so the FOV-saturated regime is
tested too.

This harness is the only one where the flow ROI comes from the REAL detector
rather than a synthetic box, which is why it carries the reduction comparison:
it is the only place the reductions can be scored under a drifting ROI and under
FOV saturation.

Run from the repo root:

    python -m bee_control.tests._optFlow_targetAcqu_debug
"""
...
# Siblings in bee_control/tests/ -- relative imports are correct here.
...
def _render_expanding_circle_target(
	...
	is exactly expected_lambda [1/s].
	"""
	...
	# radius(t) = r0 * exp(lambda * t)  =>  rdot/r = lambda exactly.
	#
	# This used to read exp(0.5 * expected_divergence * t), i.e. the parameter
	# was the FULL 2-D divergence 2*lambda -- correct when the estimator still
	# returned a1 + b2, wrong ever since it was changed to return lambda. The
	# reference line in the history panel was therefore drawn twice as high as
	# the value plotted against it, which reads as a permanent ~50% under-read.
	radius = float(initial_radius_px * np.exp(expected_lambda * phase_t))
...
def _make_target_estimate_panel(target_debug: dict, synthetic_meta: Optional[dict] = None) -> np.ndarray:
	...
	# _label_tile paints a 28 px header bar; start below it.
	y = 46
	for line in lines:
...
def _make_divergence_history_panel(history: list[dict], expected_lambda: float) -> np.ndarray:
	...
	cv2.line(panel, (left, y_of(expected_lambda)), (right, y_of(expected_lambda)), (80, 180, 255), 1)
	cv2.putText(panel, f"expected {expected_lambda:.2f}", (left + 4, y_of(expected_lambda) - 4), cv2.FONT_HERSHEY_SIMPLEX, 0.42, (80, 180, 255), 1, cv2.LINE_AA)
...
def _make_combined_canvas(
	...
	history_panel = _make_divergence_history_panel(
		history,
		expected_lambda=float(synthetic_meta.get("expected_lambda", 0.0)),
	)
	target_panel = _make_target_estimate_panel(target_debug, synthetic_meta)
	flow_panel = _make_flow_result_panel(flow_debug)

	# The raw BGR frame used to occupy tile 1, which duplicated tile 2 (the same
	# frame with the detection drawn on it). The reduction comparison earns the
	# slot instead: this harness is the only one where the ROI comes from the
	# REAL detector rather than a synthetic box, so it is the only place the
	# reductions can be compared under a drifting ROI and FOV saturation.
	comparison_panel = make_comparison_table(
		comparison,
		width=420,
		height=280,
		title="Reduction vs synthetic truth",
	)

	tiles = [
		("1 Reduction comparison", comparison_panel),
	...
	# Own strip, not an overlay: drawn on top of row_2 this clipped the last
	# line of whichever panel it landed on (the expected-lambda legend and the
	# synthetic radius readout both disappeared under it).
	footer = np.zeros((30, canvas.shape[1], 3), dtype=np.uint8)
	cv2.putText(
		footer, mode_text, (16, 20),
...
def test():
	...
	Keyboard controls:
		...
		[ and ]   -> decrease/increase expected synthetic lambda
	...
	comparison = EstimatorComparison(history_length=history_max_len)
	...
	print("[ and ]   -> decrease/increase expected synthetic lambda")
	...
	while True:
		...
		# The generator drives radius(t) = r0 * exp(0.5 * D * t), so the
		# normalized-image lambda is expected_lambda by construction. That
		# makes this the one harness where the reductions are scored against a
		# truth that survives FOV saturation.
		if flow is not None and flow.valid:
			comparison.update(
				evaluate_estimators(flow_estimator, flow, flow_debug),
				float(synthetic_meta.get("expected_lambda", float("nan"))),
				flow.fit_quality,
			)
		...
		mode_text = (
			f"synthetic pure zoom | expected_lambda={expected_lambda:.2f} 1/s | "
		...
		if key == ord("["):
			expected_lambda = max(0.10, expected_lambda - 0.10)
			flow_estimator.reset()
			target_detector.reset()
			history.clear()
			comparison.reset()
		...
		if key == ord("]"):
			expected_lambda = min(3.00, expected_lambda + 0.10)
			flow_estimator.reset()
			target_detector.reset()
			history.clear()
			comparison.reset()
