...
class WindCase:
    """One wind condition with no separate dominant-gust component.

    turbulence_* is retained as the historical campaign field name for the
    background plugin synthesis tuple
...
_BACKGROUND_BAND_HZ = (0.03, 0.12)

WIND_CASES = (
    WindCase("ww1", "weak", (2.0, 0.0, 0.0),
             turbulence_x=(3, 0.10, 0.30, *_BACKGROUND_BAND_HZ),
             turbulence_y=(2, 0.05, 0.15, *_BACKGROUND_BAND_HZ)),
    WindCase("ww2", "weak", (4.0, 0.0, 0.0),
             turbulence_x=(3, 0.20, 0.50, *_BACKGROUND_BAND_HZ),
             turbulence_y=(2, 0.10, 0.25, *_BACKGROUND_BAND_HZ)),
    WindCase("ww3", "weak", (6.0, 0.0, 0.0),
             turbulence_x=(3, 0.40, 0.80, *_BACKGROUND_BAND_HZ),
             turbulence_y=(2, 0.15, 0.35, *_BACKGROUND_BAND_HZ)),

    # Former strong-gust families, retained under the same names for direct
    # Campaign 02 comparison but with the explicit dominant gust removed.
    # ws1 is therefore a second seeded realization of the ww3 envelope.
    WindCase("ws1", "strong", (6.0, 0.0, 0.0),
             turbulence_x=(3, 0.40, 0.80, *_BACKGROUND_BAND_HZ),
             turbulence_y=(2, 0.15, 0.35, *_BACKGROUND_BAND_HZ)),
    WindCase("ws2", "strong", (3.0, 0.0, 0.0),
             turbulence_x=(3, 0.60, 1.00, *_BACKGROUND_BAND_HZ),
             turbulence_y=(2, 0.20, 0.40, *_BACKGROUND_BAND_HZ)),
...
def _speed_bound(mean: float, background: tuple) -> float:
    """Conservative component-wise bound for mean + background synthesis."""
    bound = abs(mean)
    if background:
        count, _, amp_hi, _, _ = background
        bound += count * amp_hi
    return bound
...
def build(base_seed: int, wind: WindCase, platform: PlatformCase,
    ...
    bound_x = _speed_bound(wind.mean_velocity[0], wind.turbulence_x)
    bound_y = _speed_bound(wind.mean_velocity[1], wind.turbulence_y)
