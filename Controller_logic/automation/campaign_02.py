"""Run Campaign 02: qualification of the retuned CENTER controller.

    python3 campaign_02.py --campaign campaign_02 --base-seed 20260921 --dry-run
    python3 campaign_02.py --campaign campaign_02 --base-seed 20260921 --limit 4
    python3 campaign_02.py --campaign campaign_02 --base-seed 20260921

The matrix
----------
4 wind cases x 6 platform cases x 2 deck radii x 1 gate setting = 48 runs,
one repetition per cell. This is deliberately NOT the final gate-validation
campaign. Its purpose is to qualify the retuned CENTER controller before the
planned 6 x 6 x 2 x 2 = 144-run validation campaign.

Wind cases are the three gust cases (ws1/ws2/ws3), which caused the Campaign 01
CENTER failures, plus ww3 as the strongest no-dominant-gust regression case.
Deck radii are 0.50 m (planned validation radius) and 0.40 m (small-target stress
case). Flight order keeps all 24 radius-0.50 m runs first, so ``--limit 24`` is a
clean primary qualification block before spending time on the radius-0.40 m stress
block. The commit gate is kept ON only: CENTER occurs before the commit decision,
so gate-OFF twins would duplicate the controller-acquisition test without adding
information about the failure mechanism this campaign is intended to qualify.

The platform still spans all six Campaign 01 translation-only sea states
(surge, sway, heave), preserving the weak/strong platform coverage.

Controller under test
---------------------
This campaign does not override controller parameters itself; it deliberately
keeps the Campaign 01 harness/scenario path unchanged. Run it only after the
retuned controller is installed. The intended qualification tune is:

* CENTER large-offset P blend floor increased from 0.45 to about 0.70;
* CENTER D branch no longer attenuated by the large-offset blend;
* CENTER timeout increased from 25 s to 30 s.

Keeping those changes in config.py/control_law.py rather than hiding them in the
campaign harness makes every generated run.json describe only the physical test
condition, exactly as in Campaign 01.
...
# with gate OFF would not help determine whether the new CENTER controller
# survives the gust cases, so keep the gate enabled for every run here.
GATES = ("on",)

# Campaign 01 showed that the dominant-gust cases were the acquisition failure
# regime. Keep all three, and retain ww3 as the strongest no-dominant-gust
# regression case so a more assertive CENTER tune cannot silently degrade the
# previously robust operating regime.
...
def plan(base_seed: int, *, repetitions: int = 1) -> list[scenario_mod.Scenario]:
    ...
    Campaign 02 is a controller-qualification campaign, not a gate ablation.
